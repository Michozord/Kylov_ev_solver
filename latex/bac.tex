...
\begin{definition}[Eigenvalue Problem of the Negative Laplace Operator]\label{def:ev problem}
    Let $\Omega \subset \R^d$ for $d=2,3$ be a bounded domain with a Lipschitz boundary. We seek eigenvalues $\omega^2 \in \R_{+}$ and eigenfunctions $u\in H^1(\Omega)$ of the negative Laplace operator with Neumann boundary conditions:
...
\end{definition}
Now we discretize the problem in space, fixing the partition $\mathcal{T}$ of $\Omega$. We introduce a discrete solution space as a finite-dimensional space of piecewise polynomials on $\Omega$.
...
\begin{equation*}
V_h := {v \in H^1(\Omega): \quad \forall T \in \mathcal{T}, , v|_T \in \mathcal{P}_p }
\end{equation*}
...
Let $\Omega$ be a bounded Lipschitz domain, as defined in Definition \ref{def:ev problem}, and $V_h$ be the discrete solution space. We seek eigenvalues $\omega_h^2 \in \R_+$ corresponding to discrete eigenfunctions $u \in V_h$, such that for all test functions $\varphi \in V_h$, the following holds:
...
Since we have reformulated the eigenvalue problem of the negative Laplacian operator (see Definition \ref{def:ev problem}) into a matrix eigenvalue problem, we recall elementary numerical algorithms to solve such problems. For proofs of convergence of these algorithms, we refer to \cite{numericsAB}.

The first algorithm provides an approximation of an eigenvector corresponding to the eigenvalue with the largest absolute value among the eigenvalues of the matrix.
...
\begin{algorithmic}
    ...
    \EndFor
    \State \textbf{Output:} $v^{(i)}$ -- approximation of an eigenvector to the eigenvalue with the largest absolute value.
    \end{algorithmic}
\end{algorithm}
\begin{theorem}
...
The second algorithm enables us to compute the entire spectrum of a matrix. 
...
\begin{algorithmic}
    ...
    \EndFor
    \State \textbf{Output:} $A^{(i)}$ -- approximation of an upper triangular matrix with eigenvalues of $A$ on the diagonal. 
    \end{algorithmic}
\end{algorithm}
...
Now we need to make an assumption that there exists a matrix $C\in \R^{N \times N}$ such that its eigenvectors are exact eigenvectors of the discrete problem \eqref{eq:matrix form}. 
...
Our goal is to compute eigenvalues $\omega^2$ of \eqref{eq:matrix form} in a given region of interest $(\omega_{\min}^2, \omega_{\max}^2)$. Therefore, we need matrix $C$ to fulfill the following four conditions.
\begin{itemize}
    \item $C$ satisfies the conditions in Proposition \ref{prop:C}.
...
\begin{proof}
    ...
    \end{align*}
    Therefore, $\mu = (\omega^2 - \rho)^{-1}$. Obviously, $|\mu|$ takes on the largest values if $\omega^2 \approx \rho$.
\end{proof}
...
\begin{lemma}
    ...
    \begin{enumerate}
        \item If $\omega^2$ is an eigenvalue of \eqref{eq:discr wave equation} corresponding to eigenvector $v$, then $v$ is also an eigenvector of $\Pi_\alpha$ corresponding to eigenvalue $\beta_\alpha(\omega)$. 
...
\begin{proof}
    ...
    Otherwise, if $\tau \geq 2/\omega$, then the roots of the characteristic equation are real:
    \begin{equation*}
...
\begin{lemma}\label{lemma:dff}
    ...
    \begin{enumerate}
        ...
        \item For $C$ defined in Proposition \ref{prop:C operator}, $C = \dff(M^{-1}S)$.
...
\begin{proof}
    ...
    \begin{align*}
        Cr &= \sum_{l=0}^{L-1} \tau\alpha(l\tau) y_l(r) = \sum_{j=1}^N \sum_{l=0}^{L-1} \tau\alpha(l\tau)q_l(\omega_j) (r^T v_j) v_j \\ &= \sum_{j=1}^{N}\dff(\omega_j)(r^Tv_j)v_j = V \mathrm{diag}\left(\dff(\omega_1), \dots, \dff(\omega_N)\right)V^Tr = \dff(M^{-1} S)r.
    \end{align*}
