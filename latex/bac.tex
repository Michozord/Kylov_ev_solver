...
Elementary numerical solvers for matrix eigenvalue problems struggle when the size of the problem becomes large. Therefore, instead of using a direct solver, we perform a Krylov iteration, project the large-scale problem onto a lower-dimensional Krylov space, and solve this reduced problem using a direct solver with lower computational costs. We seek eigenvalues in a specific region of interest, so we want the Krylov space to primarly cover the eigenspaces corresponding to the sought eigenvalues. 
