...
% TODO: Improve this intro to Chebyshev nodes
...
\begin{figure} % TODO: replace x with omega in legend
    \centering
...
\begin{theorem}
    ...
    \begin{equation}\label{eq:l2 matrix cont}
        X \Vec{\alpha} = \frac{1}{\tau} d
    \end{equation}
    with matrix $X\in \R^{L\times L}$, $X_{ij} := \langle q_i, q_j \rangle$ for all $i, j = 0, \dots, L-1$ and vector $d \in \R^L$, $d_j := \langle q_j, \gamma\rangle$ for all $j = 0, \dots, L-1$.
\end{theorem}
\begin{proof}
...
\begin{definition}\label{def:Xh}
    ...
    \begin{equation*}
        X_h := \left(h\sum_{k=0}^{K-1} q_i(\omega_k)q_j(\omega_k)\right)_{i,j = 0}^{L-1} \in \R^{L\times L} \text{ and } d_h := \left(h\sum_{k=0}^{K-1} q_i(\omega_k)\gamma(\omega_k) \right)_{i=0}^{L-1} \in \R^L.
    \end{equation*}
\end{definition}
