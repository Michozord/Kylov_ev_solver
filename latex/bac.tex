...
In this thesis we consider eigenvalue problem of the negative Laplace operator on a 2- or 3-dimensional domain with Neumann boundary conditions. We compute the eigenpairs $(\omega^2, u)$ with a Krylov eigenvalue solver based on time domain solutions, following \cite{nannen}. This problem plays significant role in the theory of wave equations and physics, e.g. for computation of the acoustic resonances.

We start with standard finite element method approach: we discretize the domain, fix the finite-dimensional discrete solution space and reformulate the eigenvalue problem to its weak form. This leads to the matrix eigenvalue problem to find eigenpairs $(\omega^2, v)$, such that $Sv = \omega^2Mv$, where matrices $S$ and $M$ are sparse, symmetric, and (semi-)positive definite. 

Elementary numerical solvers for matrix eigenvalue problems struggle, if the size of the problem becomes large. Therefore, instead of using a direct solver, we perform Krylov iteration, project the large scale problem onto lower-dimensional Krylov space, and solve this problem using a direct solver with lower computational costs. We seek eigenvalues in some region of interest, thus we want the Krylov space to cover chiefly eigenspaces corresponding to sought eigenvectors. 

As an operator for the iteration, we could use the shift-and-inverse matrix, but this would require computation of the inverse of a large matrix in each step, what is out of reach. A~remedy is to introduce operator $\Pi_\alpha$ as weighted time-integral of the solution to the semi-discrete wave equation, as well as its discrete implementable equivalent $C$, which uses a quadrature rule instead of the integral and finite difference method instead of analytical solution of the equation. 

The construction of this operator requires a suitable weight function $\alpha$ or a weight vector $\vec{\alpha}$. Crucial in the method is so called filter function $\beta_\alpha$ and its discrete counterpart $\dff$, which for arbitrary weight function $\alpha$, quantifies the correspondence between eigenvalues of the original problem and eigenvalues of the operator $C$. 



\newpage
The structure of this paper is as follows. We first summarize and present 
